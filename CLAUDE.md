...
**i18n:** Custom implementation, not a library. Default language is English (root `/`), French at `/fr/`. Translation strings live in `src/i18n/ui.ts` (both `fr` and `en` keys required). Use `useTranslations(getLangFromUrl(url))` in Astro components.
